...
/**
 * Normalize frontmatter `tools` to an ordered list of enabled tool names.
 * Supports legacy JSON array, YAML map (`Tool: true`), or empty.
 */
function normalizeTools(tools) {
  if (Array.isArray(tools)) return tools;
  if (tools && typeof tools === 'object') {
    return Object.keys(tools).filter((k) => tools[k] === true);
  }
  ...
  while (i < rawLines.length) {
    ...
    if (key === 'tools' && value === '') {
      ...
      while (i < rawLines.length) {
        ...
        const subMatch = subLine.match(/^\s+([^:]+):\s*(.+?)\s*$/);
        if (subMatch) {
          const subKey = subMatch[1].trim();
          const subVal = subMatch[2].trim().toLowerCase();
          if (subVal === 'true') {
            toolMap[subKey] = true;
          }
        }
