...
export type MutateAssignmentState = {
  error: string | null;
  success: boolean;
};
...
export async function updateAssignmentAction(
  ...
  try {
    const res = await fetch(
      `http://localhost:8080/api/v1/teachers/classes/${classId}/assignments/${assignmentId}`,
      {
        method: "PUT",
        headers: {
          Authorization: `Bearer ${token}`,
    ...
    if (!res.ok) {
      const message = await res.text();
      return { error: message || "Cập nhật bài tập thất bại!", success: false };
    }

    revalidatePath(`/teacher/classes/${classId}`);
    return { error: null, success: true };
  } catch (error) {
    console.error("[UpdateAssignment] Backend connection failed:", error);
    return { error: "Không kết nối được backend!", success: false };
  }
}
...
export async function deleteAssignmentAction(
  ...
  try {
    ...
    if (!res.ok) {
      const message = await res.text();
      return { error: message || "Xóa bài tập thất bại!", success: false };
    }

    revalidatePath(`/teacher/classes/${classId}`);
    return { error: null, success: true };
  } catch (error) {
    console.error("[DeleteAssignment] Backend connection failed:", error);
    return { error: "Không kết nối được backend!", success: false };
  }
}
